...
static class Node {
  ...
  public void add() {
    ...
    if(head == null){ //if(this.head == null)
      tail = head = node; //노드 > 헤드 > 테일 순으로 대입
    } else {
      // 기존에 tail이 가리키는 마지막 노드의 next 변수에 새 노드 주소를 저장함
      tail.next = node;
      // 새로 만든 노드를 마지막 노드로 설정함 
      tail = node;
...
public void list() {
  ...
  do {
    System.out.printf("%d, %s, %s, %s, %d, %d\n", 
        ...
        node.board.like);
    node = node.next; 
  } while(node != null); //node가 null값이 아니면 해당하는 값의 주소로 찾아가서 변수 출력(do)
}
